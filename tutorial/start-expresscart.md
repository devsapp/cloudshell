# 快速体验start-expresscart

欢迎您使用Serverless Devs开发者工具进行项目开发，本实验是基于Serverless Devs部署start-expresscart案例到阿里云函数计算。
...
> start-expresscart案例仓库地址：https://github.com/devsapp/start-expresscart
...
```
s init devsapp/start-expresscart
```

并按照引导进行项目名称的输入，例如此时输入项目名称`start-expresscart`

## 部署项目

根据上一步输入的项目名称，进入到项目，例如上一步输入的项目名称是`start-expresscart`，此时则可以`cd start-expresscart`
